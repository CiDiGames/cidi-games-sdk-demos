...
Spring Boot demo server for CIDI Game SDK server-side OpenAPI integration.
...
```bash
curl --request POST http://localhost:3003/demo/verify \
...
- This demo follows the same endpoint shape as the Node.js, Python, and Go demo servers so clients can switch demo server languages with minimal changes.
